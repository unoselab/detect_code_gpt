#!/usr/bin/env python3
"""Audit gt200 run-1a perturbation-detector inputs before NPR scoring.
...
This versioned implementation adds explicit named minimum-token
specifications. Its default configuration creates exactly one eligibility
specification named ``gt200`` with the inclusive boundary ``tokens >= 201``.
It writes to a gt200-v1 output directory and does not modify the original
run-1b strict artifacts.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("output/commit_function/run-1b/gt200-v1"),
    )
    parser.add_argument(
        "--qc-dir",
        type=Path,
    ...
    parser.add_argument(
        "--named-minimum-token-specs",
        default="gt200:201",
        help=(
            "Comma-separated NAME:MIN specifications with inclusive minimum "
            "literal-space token thresholds."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--include-all-positive",
        action=argparse.BooleanOptionalAction,
        default=False,
        help=(
            "Include the diagnostic all_positive specification. The gt200-v1 "
            "workflow disables it so the support CSV contains only gt200."
        ),
    )
...
def parse_specs(args: argparse.Namespace) -> list[EligibilitySpec]:
    ...
    if args.include_all_positive:
        specs.append(EligibilitySpec("all_positive", 1, None, "diagnostic"))
        seen_names.add("all_positive")

    named_text = str(args.named_minimum_token_specs).strip()
    if named_text:
        for raw in named_text.split(","):
            item = raw.strip()
            if not item:
                continue
            if ":" not in item:
                raise ValueError(f"Invalid named minimum-token specification: {item}")
            name, minimum_text = (part.strip() for part in item.split(":", 1))
            if not name:
                raise ValueError(
                    f"Named minimum-token specification has an empty name: {item}"
                )
            if name in seen_names:
                raise ValueError(f"Duplicate eligibility specification name: {name}")
            minimum = int(minimum_text)
            if minimum < 1:
                raise ValueError(
                    f"Minimum token threshold must be positive: {minimum}"
                )
            role = "primary_candidate" if name == args.primary_spec else "sensitivity"
            specs.append(EligibilitySpec(name, minimum, None, role))
            seen_names.add(name)
...
def analyze(args: argparse.Namespace) -> dict[str, Any]:
    ...
    metadata = {
        "status": status,
        "analysis_stage": "run-1b-input-support-preflight-gt200-v1",
        "implementation_version": "gt200-v1",
        "inputs": {
...
def run_self_test() -> None:
    with tempfile.TemporaryDirectory(prefix="run1b-self-test-") as temporary:
        ...
        named_namespace = argparse.Namespace(
            include_all_positive=False,
            named_minimum_token_specs="gt200:201",
            minimum_token_thresholds="",
            bounded_token_ranges="",
            primary_spec="gt200",
        )
        named_specs = parse_specs(named_namespace)
        if named_specs != [
            EligibilitySpec("gt200", 201, None, "primary_candidate")
        ]:
            raise AssertionError(
                f"Self-test named gt200 specification mismatch: {named_specs}"
            )
