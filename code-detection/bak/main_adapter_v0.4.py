...
To align with the project's calibration regime (generate_data + batch_benchmark):
  ...
  2. CHUNK the body into 128-"token" windows and AGGREGATE (no front-128 loss).
     - Tokenization for chunking is whitespace split (text.split(" ")), exactly
       ...
       (default 1) are low-confidence and excluded from scoring + aggregation.
...
-----
    ...
    python main_adapter.py --csv_path merged_4500.csv --drop_preflight

    python main_adapter.py --csv_path merged_4500.csv --preflight_only
...
# ---------------------------------------------------------------------------
# Fast preflight drop reporting (no model load / no NPR scoring)
# ---------------------------------------------------------------------------
def inspect_snippet_static(code, chunk_len, min_chunk_tokens, strip):
    """Return static chunk/drop info that can be known before NPR scoring.

    This mirrors score_snippet's body stripping and chunk filtering, but it never
    calls get_rank / perturb_texts / get_ranks, so it is safe to run before the
    expensive model load and NPR computation. It cannot predict later NaNs from
    the model/rank computation.
    """
    if strip:
    ...
    chunks = chunk_whitespace(body, chunk_len)
    n_scorable = 0
    n_tokens = 0
    for chunk_text, n_tok in chunks:
        n_tokens += n_tok
        if n_tok >= min_chunk_tokens and chunk_text.strip():
            n_scorable += 1

    return {
        "reason": reason,
        "n_chunks": len(chunks),
        "n_scorable_chunks": n_scorable,
        "n_lowconf_chunks": len(chunks) - n_scorable,
        "n_tokens": n_tokens,
        "dropped": n_scorable == 0,
    }


def print_drop_preflight_report(pairs, chunk_len, min_chunk_tokens, strip, max_examples):
    """Print how many snippets/pairs will be statically unscorable."""
    reason_counts = {}
    n_human_dropped = 0
    n_lm_dropped = 0
    n_pair_dropped = 0
    n_human_lowconf_chunks = 0
    n_lm_lowconf_chunks = 0
    examples = []

    for line_num, human_code, lm_code in pairs:
        h = inspect_snippet_static(human_code, chunk_len, min_chunk_tokens, strip)
        l = inspect_snippet_static(lm_code, chunk_len, min_chunk_tokens, strip)

        reason_counts[h["reason"]] = reason_counts.get(h["reason"], 0) + 1
        reason_counts[l["reason"]] = reason_counts.get(l["reason"], 0) + 1
        n_human_lowconf_chunks += h["n_lowconf_chunks"]
        n_lm_lowconf_chunks += l["n_lowconf_chunks"]
        ...
        if h_drop or l_drop:
            ...
            if len(examples) < max_examples:
                examples.append((line_num, h, l))

    n_pairs = len(pairs)
    n_snippets = 2 * n_pairs
    n_snippet_dropped = n_human_dropped + n_lm_dropped

    print()
    print("=" * 72)
    print("Fast static drop preflight (before model load / NPR scoring)")
    print("=" * 72)
    print(f"Pairs loaded: {n_pairs}")
    print(f"Snippets inspected: {n_snippets}")
    print(f"Human snippets with 0 statically scorable chunks: {n_human_dropped}")
    print(f"LM snippets with 0 statically scorable chunks: {n_lm_dropped}")
    print(f"Total snippets with 0 statically scorable chunks: {n_snippet_dropped}")
    print(f"Pairs that would be dropped before AUROC for this reason: {n_pair_dropped}")
    print(f"Pairs remaining after this static filter: {n_pairs - n_pair_dropped}")
    print(f"Low-confidence chunks excluded before scoring: "
          f"human={n_human_lowconf_chunks}, lm={n_lm_lowconf_chunks}")
    print()
    print("Body-extraction outcomes (per snippet):")
    for reason, count in sorted(reason_counts.items()):
        print(f"  {reason}: {count}")

    if examples:
        print()
        print(f"First {len(examples)} statically dropped pair example(s):")
        for line_num, h, l in examples:
            h_flag = "DROP" if h["dropped"] else "keep"
            l_flag = "DROP" if l["dropped"] else "keep"
            print(
                f"  line{line_num}: "
                f"human={h_flag}({h['reason']}, scorable={h['n_scorable_chunks']}, "
                f"lowconf={h['n_lowconf_chunks']}, tokens={h['n_tokens']}); "
                f"lm={l_flag}({l['reason']}, scorable={l['n_scorable_chunks']}, "
                f"lowconf={l['n_lowconf_chunks']}, tokens={l['n_tokens']})"
            )

    print()
    print("Note: this preflight does not count later NaNs from get_rank / "
          "perturb_texts / get_ranks; those can only be known during NPR scoring.")
    print("=" * 72)
    print()
...
def main():
    ...
    parser.add_argument("--drop_preflight", action="store_true",
                        help="Before model loading/scoring, print how many snippets/pairs "
                             "have no statically scorable chunks after body stripping and "
                             "chunk-length filtering.")
    parser.add_argument("--preflight_only", action="store_true",
                        help="Run the drop preflight report and exit before model loading "
                             "or NPR scoring.")
    parser.add_argument("--preflight_examples", type=int, default=10,
                        help="Maximum number of dropped pair examples to print in preflight reports.")
    cli = parser.parse_args()

    if cli.chunk_len < 1:
        parser.error("--chunk_len must be >= 1")
    if cli.min_chunk_tokens < 1:
        parser.error("--min_chunk_tokens must be >= 1")
    if cli.n_perturbation < 1:
        parser.error("--n_perturbation must be >= 1")
    if cli.preflight_examples < 0:
        parser.error("--preflight_examples must be >= 0")
    ...
    if cli.drop_preflight or cli.preflight_only:
        print_drop_preflight_report(
            pairs,
            chunk_len=cli.chunk_len,
            min_chunk_tokens=cli.min_chunk_tokens,
            strip=cli.strip_body,
            max_examples=cli.preflight_examples,
        )
