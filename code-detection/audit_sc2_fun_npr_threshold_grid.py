#!/usr/bin/env python3
"""Audit the FUN file-NPR threshold grid using the frozen C01 SC2-7B threshold.

C02 consumes the frozen A12 repo-month/Python-file FUN NPR artifact and transfers
the detector threshold selected by C01 to the continuous historical FUN signal.
...
Treatment timing
----------------
C02 follows the normalized Model A timing used by the quality DiD design:
...
Outputs are aggregate audits only.  C02 does not generate 23 duplicated
file-level classification datasets; the next FUN quality stage can reapply the frozen threshold spec
...
def validate_a11_lineage(results_root: Path, cache_ref_file: Path) -> dict[str, Any]:
    records: list[dict[str, Any]] = []
    for gpu_index in range(3):
        metadata_path = results_root / f"gpu-{gpu_index}" / "metadata.json"
        if not metadata_path.is_file():
            raise FileNotFoundError(metadata_path)
        metadata = load_json(metadata_path, f"A11 GPU {gpu_index} metadata")
        scoring = metadata.get("scoring_configuration") or {}
        runtime = metadata.get("runtime") or {}
        inputs = metadata.get("inputs") or {}
        expected_pairs = [
            ("script_version", metadata.get("script_version"), EXPECTED_A11_SCRIPT_VERSION),
        ...
        for key, observed, expected in expected_pairs:
            if observed != expected:
                raise ValueError(
                    f"A11 GPU {gpu_index} metadata mismatch for {key}: observed={observed!r}, expected={expected!r}"
                )
        records.append(
            {
                "gpu_index": gpu_index,
    ...
    return {
        ...
        "gpu_metadata": records,
        "note": (
            "A11 metadata is the historical model-revision provenance. The current Hugging Face cache ref is recorded "
            "as a reproducibility consistency gate; it is not treated as retroactive proof of the C01 run revision."
        ),
    }
...
def make_checks(
    ...
    add_check(checks, "threshold_total_count_with_anchors", len(specs) == 23, len(specs), 23, "C02 must have the 21-point grid plus the legacy and prior-primary named anchors.")
    ...
    add_check(checks, "unexpected_file_status_rows", diagnostics["unexpected_status_rows"] == 0, diagnostics["unexpected_status_rows"], 0, "C02 accepts only A12 v2 file-status values.")
...
def self_test() -> None:
    ...
    print("audit_sc2_fun_npr_threshold_grid self-test: PASS")
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--a11-results-root", type=Path, help="A11 results root containing gpu-0, gpu-1, and gpu-2 metadata")
    parser.add_argument("--sc2-cache-ref-file", type=Path, help="Hugging Face refs/main file for bigcode/starcoder2-7b")
    parser.add_argument("--output-dir", type=Path, help="C02 output directory")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    required = {
        ...
        "--a12-summary-file": args.a12_summary_file,
    ...
    for path in (args.input_file, args.a12_summary_file, args.c01_threshold_spec_file, args.c01_summary_file, args.sc2_cache_ref_file):
        if not path.is_file():
    ...
    if args.primary_threshold != DEFAULT_PRIMARY_THRESHOLD:
        raise ValueError(f"C02 v3 freezes primary threshold at {DEFAULT_PRIMARY_THRESHOLD}; observed override={args.primary_threshold}")
    if args.grid_step != DEFAULT_GRID_STEP:
        raise ValueError(f"C02 v3 freezes grid step at {DEFAULT_GRID_STEP}; observed override={args.grid_step}")
    if args.grid_radius != DEFAULT_GRID_RADIUS:
        raise ValueError(f"C02 v3 freezes grid radius at {DEFAULT_GRID_RADIUS}; observed override={args.grid_radius}")
    if args.legacy_threshold != DEFAULT_LEGACY_THRESHOLD:
        raise ValueError(f"C02 v3 freezes legacy threshold at {DEFAULT_LEGACY_THRESHOLD}; observed override={args.legacy_threshold}")
    if args.prior_primary_threshold != DEFAULT_PRIOR_PRIMARY_THRESHOLD:
        raise ValueError(f"C02 v3 freezes prior paper primary threshold at {DEFAULT_PRIOR_PRIMARY_THRESHOLD}; observed override={args.prior_primary_threshold}")
    ...
    a11_lineage = validate_a11_lineage(args.a11_results_root, args.sc2_cache_ref_file)
    ...
    summary = {
        ...
        "methodology": {
            ...
            "threshold_selection_note": "C01 selected the detector threshold from detector-benchmark data only; C02 transfers that frozen threshold without reading SonarQube or any quality outcome.",
        },
        "input": diagnostics,
        "a12": {
    ...
    print("=" * 80)
    print("run-x-c02 FUN file-NPR threshold-grid audit")
