...
def load_base_model_and_tokenizer(args, model_config):
    ...
    if '13b' in name:
        ...
        # 2026-06-08 msong: Gemma 4 31B does not fit on one 48GB A6000.
        # The generic loader below loads on CPU and then calls .to(cuda), which
        # attempts to move the entire model to CUDA device 0 and OOMs. Mirror the
        # GPT-OSS/StarCoder large-model pattern: use Accelerate device_map="auto"
        # with explicit per-GPU memory caps, and DO NOT call base_model.to(...)
        # after from_pretrained(..., device_map="auto").
        print(f"CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES')}")
        print(f"torch.cuda.device_count()={torch.cuda.device_count()}")
        ...
        if n_gpu == 0:
            raise RuntimeError(
                "No CUDA GPU visible for Gemma scoring. "
                f"CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES')}"
            )
        ...
        for i, mem in enumerate(gpu_mem_gib):
            if is_large_gemma and two_large_48g_gpus:
                ...
                max_memory[i] = f"{int(max(1, mem - 4))}GiB"
        max_memory["cpu"] = os.environ.get("GEMMA_CPU_MAX_MEMORY", "128GiB")

        print(
            f"Using Gemma multi-GPU loader; visible GPUs={n_gpu}, "
            f"dtype={dtype}, max_memory={max_memory}"
        )
