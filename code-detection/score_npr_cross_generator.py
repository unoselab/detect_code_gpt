#!/usr/bin/env python3
"""score_npr_cross_generator-v1.py

Evaluate overlap-window DetectCodeGPT NPR across mixed-code generation sources.

This experiment separates two roles that were matched in the original same-source
benchmark:
...
main_mixedcode_benchmark_overlap.py implementation so diagonal cells can serve as
...
Typical usage from code-detection/ after removing the development version suffix:

  CUDA_VISIBLE_DEVICES=0 python score_npr_cross_generator.py \
    --target_source codellama-7b \
    ...
    --output_name npr-xgen_score-starcoder2-7b_target-codellama-7b

Fast re-aggregation without rescoring remains supported through
--load_cached_results.
"""
...
def score_examples(
    ...
    for ex in tqdm(examples, desc="Scoring function bodies"):
        ...
        r = dict(ex)
        # Keep explicit cross-generator provenance in every cached/output row.
        r["target_source"] = cli.target_source
...
def main() -> None:
    ...
    parser.add_argument("--target_source", required=True,
                        help="Short label for the AGC generation source used by this benchmark.")
    parser.add_argument("--scoring_model_key", required=True,
    ...
    print("=" * 80)
    print("score_npr_cross_generator-v1.py")
    print("=" * 80)
    print(f"target_source        : {cli.target_source}")
