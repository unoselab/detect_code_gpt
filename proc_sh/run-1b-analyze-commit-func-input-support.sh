#!/usr/bin/env bash
# Audit perturbation-detector commit-function inputs before NPR scoring.
#
# Workspace:
#   ~/project-workspace/detect_code_gpt
#
# Versioned delivery file:
#   proc_sh/run-1b-analyze-commit-func-input-support-v1.sh
#
# Canonical path:
#   proc_sh/run-1b-analyze-commit-func-input-support.sh
#
# Purpose:
#   Analyze the completed run-1a outputs before StarCoder2-7B scoring by:
...
#   - comparing pre-specified eligibility rules by cohort and period,
...
#     candidate or frozen scoring specification
...
#   OUTPUT_DIR, QC_DIR, LOG_DIR, MINIMUM_TOKEN_THRESHOLDS,
#   BOUNDED_TOKEN_RANGES, PRIMARY_SPEC, WINDOW_SIZE,
...
# 
# Usage:
# PYTHON_BIN=/home/user1-system12/miniconda3/envs/agcparse312/bin/python OVERWRITE_OUTPUT=1 bash proc_sh/run-1b-analyze-commit-func-input-support.sh
# 
# PYTHON_BIN=/home/user1-system12/miniconda3/envs/agcparse312/bin/python PRIMARY_SPEC=range100_200 FREEZE_SPECIFICATION=1 OVERWRITE_OUTPUT=1 bash proc_sh/run-1b-analyze-commit-func-input-support.sh
...
PY_SCRIPT="${PY_SCRIPT:-code-detection/analyze_commit_function_input_support.py}"
...
OUTPUT_DIR="${OUTPUT_DIR:-output/commit_function/run-1b/strict}"
QC_DIR="${QC_DIR:-${OUTPUT_DIR}/qc}"
LOG_DIR="${LOG_DIR:-logs/run-1b}"

MINIMUM_TOKEN_THRESHOLDS="${MINIMUM_TOKEN_THRESHOLDS:-36,78,100}"
BOUNDED_TOKEN_RANGES="${BOUNDED_TOKEN_RANGES:-100:200}"
PRIMARY_SPEC="${PRIMARY_SPEC:-min100}"
...
FREEZE_SPECIFICATION="${FREEZE_SPECIFICATION:-0}"
OVERWRITE_OUTPUT="${OVERWRITE_OUTPUT:-0}"
RUN_SELF_TEST="${RUN_SELF_TEST:-1}"

TIMESTAMP="${TIMESTAMP:-$(date +%Y%m%d-%H%M%S)}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/run-1b-analyze-commit-func-input-support-${TIMESTAMP}.log}"
...
cat <<INFO
============================================================================
run-1b: analyze perturbation-detector commit-function input support
Started:                         ${START_TEXT}
...
Primary specification candidate: ${PRIMARY_SPEC}
