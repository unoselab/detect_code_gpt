#!/usr/bin/env bash
# run-x-b01-v6: exact-tie/engine-real profiling and B=1 GPT-OSS historical NPR
# optimization, plus a diagnostic-only per-device pipeline-stage timing probe.
#
# This standalone wrapper was created by copying the prior B01 wrapper logic and
# adapting it for the v6 Python experiment. It never calls an older shell script.
...
#
# DELIVERY (remove -v6 when deploying to canonical server names):
#   proc_sh/run-x-b01-score-historical-npr-gptoss-v6.sh
...
#
# ACTUAL r158 v5 PROFILE RESULT (5 windows / 255 sequences; kept here so the next
# operator does not have to re-derive it): forward=90.4%, legacy_argsort=3.0%,
# legacy_sleep=6.3% (dead time.sleep(0.01) in the frozen rank.py reference path;
# the real MODE=run engine never pays it), tokenize/h2d/reduction <0.3% combined.
# count-index-asc and count-priority reproduced legacy argsort's integer rank
# exactly (0 mismatches); count-index-desc did not. Best exact candidate
# (count-priority) is only ~1.30x on the argsort sub-stage alone, so the rank-only
# Amdahl ceiling is ~1.03x (measured: 82.55 -> 80.05 days) -- nowhere near the
# 5-7 day conference target. v6 does not chase this further; see MODES below.
#
# MODES:
...
#     real logits, test exact O(V) tie-order hypotheses. Also reports the
#     engine-real (no-sleep) rate/ETA actually achievable by MODE=run's hot path,
#     and (best-effort, diagnostic-only, never fails the stage) a per-device
#     forward-stage timing probe that attributes the dominant forward cost across
#     pipeline-parallel GPU stages -- see DEVICE_STAGE_PROFILE below. No production
#     approval is granted here; `next_step` in profile_summary.json only turns
#     "validate" when the profile's own numbers show that step could plausibly
#     reach the hard budget, so a GPU-expensive validate run is not recommended
#     on numbers that already prove it futile.
#   validate:
...
#   finalize:
#     Rebuild exports from an existing v6 production checkpoint without loading GPT-OSS.
#
# FIRST COMMAND ON r158 (re-profiling from scratch, e.g. a new host/topology):
#   MODE=profile CUDA_DEVICE=0,1,2 PROFILE_WINDOWS=5 bash proc_sh/run-x-b01-score-historical-npr-gptoss-v6.sh
#
# IMPORTANT:
#   v4 established that GPT-OSS batch size >1 changes the numerical result. v5/v6
#   therefore fix BATCH_SIZES=1. v6 does not add a new execution path for production;
#   it only adds engine-real ETA reporting and a read-only device-stage diagnostic to
#   correctly point a future v7 (forward-pass/pipeline optimization) at real data.
#   No classification/thresholding is performed in B01; tau=1.545529 is downstream.
#   v2 smoke and A09 inputs are read-only. v6 writes only under run-x-b01/v6 by default.
...
PY_SCRIPT="${PY_SCRIPT:-code-detection/score_historical_npr_gptoss-v6.py}"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-output/snapshot_npr/run-x-b01/v6}"
...
DEVICE_STAGE_PROFILE="${DEVICE_STAGE_PROFILE:-1}"
DEVICE_STAGE_PROFILE_FORWARDS="${DEVICE_STAGE_PROFILE_FORWARDS:-3}"
...
if [[ "${OVERWRITE}" != "0" && "${MODE}" != "run" ]]; then
  echo "ERROR: OVERWRITE=1 is only valid for an explicitly fresh v6 production checkpoint." >&2; exit 2
fi
...
LOG_FILE="${LOG_FILE:-${LOG_DIR}/${RUN_PREFIX}-v6-${MODE}-${SYSTEM_LABEL}-${TIMESTAMP}.log}"
mkdir -p "${LOG_DIR}"
require_file() {
    local path="$1"
    local label="$2"
...
require_file "${PY_SCRIPT}" "B01-v6 Python"
...
case "${MODE}" in
  ...
  validate) require_file "${REFERENCE_DB}" "v2 smoke checkpoint (read-only)"; require_file "${PROFILE_SUMMARY}" "passing v6 profile" ;;
  benchmark) require_file "${VALIDATION_SUMMARY}" "passing v6 validation" ;;
...
finish() {
    ...
    echo
    echo "============================================================================"
    echo "run-x-b01-v6 execution summary"
    echo "Mode:             ${MODE}"
...
echo "============================================================================"
echo "run-x-b01-v6: exact-tie/engine-real profiling and B=1 GPT-OSS historical NPR optimization"
echo "Mode:                            ${MODE}"
...
echo "Device-stage diagnostic:         ${DEVICE_STAGE_PROFILE} (forwards=${DEVICE_STAGE_PROFILE_FORWARDS}; diagnostic-only, never fails the stage)"
...
if [[ "${RUN_SELF_TEST}" == "1" ]]; then
  "${PYTHON_BIN}" "${PY_SCRIPT}" --project-root "${PROJECT_ROOT}" --a02-script "${A02_SCRIPT}" \
    --rank-script "${RANK_SCRIPT}" --output-dir "/tmp/run-x-b01-v6-self-test" --self-test-only
fi
ARGS=(
  ...
  --device-stage-profile-forwards "${DEVICE_STAGE_PROFILE_FORWARDS}"
...
if [[ "${DEVICE_STAGE_PROFILE}" == "1" ]]; then ARGS+=(--device-stage-profile); else ARGS+=(--no-device-stage-profile); fi
"${PYTHON_BIN}" "${PY_SCRIPT}" "${ARGS[@]}"
require_file "${OUTPUT_DIR}/summary.json" "v6 stage summary"
...
if summary.get("stage") == "profile":
    print("Next step:", summary.get("next_step"))
    print("Feasibility:", summary.get("feasibility_note"))
    print("Engine-real ETA (days):", summary.get("engine_real_eta_days"))
    print("Engine + recommended-rank ETA (days):", summary.get("engine_with_recommended_rank_eta_days"))
    diag = summary.get("device_stage_diagnostic") or {}
    print("Device-stage diagnostic status:", diag.get("status"))
