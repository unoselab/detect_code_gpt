#!/usr/bin/env bash
# Audit the FUN file-NPR threshold grid using the frozen C01 pooled SC2-7B threshold.
#
# This wrapper is standalone. It was copied from the validated run-x-d01 FUN threshold-audit
# wrapper and adapted for the C01 pooled SC2-7B threshold. It does not call or depend on any
# previous shell wrapper.
#
# Versioned delivery files:
#   code-detection/audit_sc2_fun_npr_threshold_grid-v3.py
#   proc_sh/run-x-c02-audit-fun-npr-threshold-grid-v3.sh
#
# Canonical server paths after deployment:
#   code-detection/audit_sc2_fun_npr_threshold_grid.py
#   proc_sh/run-x-c02-audit-fun-npr-threshold-grid.sh
#
# Required inputs:
#   output/snapshot_npr/run-x-a12/python_fun_repo_month_file_npr_scores.csv
#       A12 repo-month/Python-file FUN NPR artifact from the same detect_code_gpt workspace.
#   output/snapshot_npr/run-x-a12/summary.json
#       A12 terminal summary. C02 requires run-x-a12-v2 with zero hard QC failures.
#
...
#   C02 validates C01 model, population, algorithm, and threshold fields before classification.
#
# Historical scorer provenance:
#   output/snapshot_npr/run-x-a11/results/gpu-{0,1,2}/metadata.json
...
#
# Main outputs under output/snapshot_npr/run-x-c02/fun-threshold-v3/:
...
#
# Important boundary:
#   C01 selected the threshold from detector-benchmark data only. C02 does not read SonarQube
#   or any quality outcome; it transfers the already-frozen C01 threshold to historical FUN NPR.
#
# Runtime:
...
RUN_PREFIX="run-x-c02"
IMPLEMENTATION_VERSION="v3"
RUN_LABEL="${RUN_PREFIX}-${IMPLEMENTATION_VERSION}"
RUN_TS="${RUN_TS:-$(date +%Y%m%d-%H%M%S)}"

PYTHON_BIN="${PYTHON_BIN:-python}"
PY_SCRIPT="${PY_SCRIPT:-code-detection/audit_sc2_fun_npr_threshold_grid.py}"
A12_INPUT_FILE="${A12_INPUT_FILE:-output/snapshot_npr/run-x-a12/python_fun_repo_month_file_npr_scores.csv}"
A12_SUMMARY_FILE="${A12_SUMMARY_FILE:-output/snapshot_npr/run-x-a12/summary.json}"
...
OUTPUT_DIR="${OUTPUT_DIR:-output/snapshot_npr/run-x-c02/fun-threshold-v3}"
LOG_DIR="${LOG_DIR:-logs/run-x-c02}"
...
for required_file in "${PY_SCRIPT}" "${A12_INPUT_FILE}" "${A12_SUMMARY_FILE}" "${C01_THRESHOLD_SPEC_FILE}" "${C01_SUMMARY_FILE}" "${SC2_CACHE_REF_FILE}"; do
...
if [[ "${PYTHON_MAJOR_MINOR}" != "3.11" ]]; then
  echo "ERROR: C02 requires Python 3.11.x; found ${PYTHON_VERSION}" >&2
  exit 2
fi
...
START_EPOCH="$(date +%s)"
echo "============================================================================"
echo "${RUN_LABEL}: audit FUN file-NPR grid with frozen C01 SC2-7B threshold"
echo "Started:                         $(date)"
